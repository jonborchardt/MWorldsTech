using System;
using System.Collections.Generic;

namespace GameNet.Utils
{
    public static class TextCubes
{
    private const int CharWidth = 5;
    private const int CharHeight = 7;

    // Classic Adafruit GFX 5x7 font: 256 chars * 5 bytes/char = 1280 bytes.
    // Each byte is one column; bits are the pixels (we use bits 0..6 for 7 rows).
    // Source: glcdfont.c font[] table. :contentReference[oaicite:1]{index=1}
    private static readonly byte[] Font = new byte[]
    {
        
    };

    public static List<Dictionary<string, object>> GetTextCubes(
        string text,
        float startX = 0f,
        float startY = 1f,
        float startZ = 0f,
        float pixelSize = 1f,
        float charSpacing = 1f,
        float lineSpacing = 2f,
        float rotX = 0f,
        float rotY = 0f,
        float rotZ = 0f,
        string prefabId = "Cube",
        string stateJson = "",
        string stableIdPrefix = "cube"
    )
    {
        var cubes = new List<Dictionary<string, object>>();
        var nextStableId = MakeStableIdFactory(stableIdPrefix);

        var x = startX;
        var z = startZ;

        for (var i = 0; i < text.Length; i += 1)
        {
            var c = text[i];

            if (c == '\n')
            {
                x = startX;
                z -= (CharHeight * pixelSize) + (lineSpacing * pixelSize);
                continue;
            }

            cubes.AddRange(GetCharCubes(
                c,
                x,
                startY,
                z,
                nextStableId,
                pixelSize,
                rotX,
                rotY,
                rotZ,
                prefabId,
                stateJson
            ));

            x += (CharWidth * pixelSize) + (charSpacing * pixelSize);
        }

        return cubes;
    }

    public static List<Dictionary<string, object>> GetCharCubes(
        char c,
        float originX,
        float originY,
        float originZ,
        Func<string> nextStableId,
        float pixelSize = 1f,
        float rotX = 0f,
        float rotY = 0f,
        float rotZ = 0f,
        string prefabId = "Cube",
        string stateJson = ""
    )
    {
        var cubes = new List<Dictionary<string, object>>();

        var code = (int)c;

        // This font table is 0..255. For anything outside, render as '?' (63) if possible.
        if (code < 0 || code > 255)
        {
            code = 63;
        }

        var baseIndex = code * 5;

        for (var col = 0; col < CharWidth; col += 1)
        {
            var columnBits = Font[baseIndex + col];

            for (var row = 0; row < CharHeight; row += 1)
            {
                // bit 0..6 used for 7 rows
                var on = ((columnBits >> row) & 0x01) == 1;
                if (!on)
                {
                    continue;
                }

                var posX = originX + (col * pixelSize);

                // Make row 0 be "top" visually:
                // We treat row 0 as top, so invert into Z.
                var posZ = originZ + ((CharHeight - 1 - row) * pixelSize);

                cubes.Add(new Dictionary<string, object>
                {
                    { "stableId", nextStableId() },
                    { "prefabId", prefabId },
                    { "posX", posX },
                    { "posY", originY },
                    { "posZ", posZ },
                    { "rotX", rotX },
                    { "rotY", rotY },
                    { "rotZ", rotZ },
                    { "stateJson", stateJson }
                });
            }
        }

        return cubes;
    }

    private static Func<string> MakeStableIdFactory(string prefix)
    {
        var i = 0;
        return () =>
        {
            i += 1;
            return $"{prefix}-{i:D8}";
        };
    }
    }
}
